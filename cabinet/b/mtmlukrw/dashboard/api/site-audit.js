...
import { DomainError } from '../core/types.js?v=mtmlukrw';
...
// Границы слова обязательны (урок tenderdoki.ru: «подТВЕРдите» ловилось как Тверь,
// «сЧИТаем» — как Чита); после основы допускается только падежное окончание.
function cityRegex(city        )         {
  const parts = city.split(/[-\s]/);
  const last = parts[parts.length - 1];
  let stem = last; let endings          ;
  if (/[ая]$/i.test(last)) { stem = last.slice(0, -1); endings = ['а', 'я', 'е', 'ы', 'и', 'у', 'ю', 'ой', 'ей', 'ью']; }
  else if (/ь$/i.test(last)) { stem = last.slice(0, -1); endings = ['ь', 'и', 'ью']; }
  else if (/о$/i.test(last)) { stem = last.slice(0, -1); endings = ['о', 'е', 'а']; }
  else if (/[еи]$/i.test(last)) { endings = ['']; }
  else { endings = ['', 'а', 'е', 'у', 'ом']; }
  const prefix = parts.slice(0, -1).map((p) => p + '[-\\s]').join('');
  return new RegExp(`(^|[^а-яёa-z])${prefix}${stem}(?:${endings.join('|')})(?=[^а-яёa-z]|$)`, 'gi');
}

export function detectGeography(text        )                {
  // Реквизиты (ИНН, юр. адрес «644103, г. Омск») — не география работы.
  const cleaned = text.replace(/(ИНН|ОГРН|КПП)\s*\d[\d\s]*/gi, ' ').replace(/\d{6},?\s*г\.?\s*[А-ЯЁ][а-яё-]+/g, ' ');
  if (/все[йю]\s+Росси[ию]|вся\s+Россия|по\s+России/i.test(cleaned)) return 'вся Россия';
  const found                                    = [];
  for (const city of RU_CITIES) {
    const m = cleaned.match(cityRegex(city));
    if (m) found.push({ city, count: m.length });
  }
  found.sort((a, b) => b.count - a.count);
  const russiaCount = (cleaned.match(/(^|[^а-яёa-z])Росси[ияюей]/gi) ?? []).length;
  // Сайт много говорит про Россию, а города упоминает мельком (примеры) — значит, вся страна.
  if (russiaCount >= 2 && (!found.length || found[0].count < 3)) return 'вся Россия';
  if (found.length) return found.slice(0, 2).map((f) => f.city).join(', ');
  if (russiaCount) return 'вся Россия';
  return null;
}

// Аудитория по тематике сайта (замечание владельца 03.09: «он сам должен понять, какая
// аудитория»). Честный словарь ниш: по словам на странице определяется тематика,
// для неё — типовые сегменты клиентов. Не выдумывание, а подбор по маркерам; в мастере
// помечается как подобранное — владелец уточняет.
const NICHES                                                           = [
  { label: 'тендеры и закупки', markers: ['тендер', 'закупк', 'госзакуп', '44-фз', '223-фз', 'аукцион', 'котировк'], audience: 'компании, участвующие в госзакупках и коммерческих тендерах; тендерные специалисты и менеджеры по закупкам; руководители малого и среднего бизнеса, которые выходят на тендеры без штата экспертов' },
  { label: 'переработка и вторсырьё', markers: ['отход', 'металлолом', 'вторсырь', 'утилизац', 'лом ', 'нефтепродукт', 'макулатур'], audience: 'предприятия, которые сдают отходы, лом и неликвиды; снабженцы и экологи производств; покупатели вторсырья и б/у материалов' },
  { label: 'ремонт и строительство', markers: ['ремонт квартир', 'строительств', 'отделк', 'под ключ', 'сантехник', 'электромонтаж'], audience: 'владельцы квартир и домов, планирующие ремонт; заказчики строительства; управляющие коммерческими объектами' },
  { label: 'красота', markers: ['салон красоты', 'маникюр', 'парикмахер', 'косметолог', 'барбершоп', 'эпиляц'], audience: 'клиенты рядом с салоном, которые выбирают мастера по работам и отзывам; постоянные гости по записи и абонементам' },
  { label: 'медицина', markers: ['стоматолог', 'клиник', 'врач', 'медицинск', 'лечени', 'диагностик'], audience: 'пациенты, выбирающие врача по отзывам и расположению; семьи с детьми; постоянные пациенты по программам' },
  { label: 'юридические услуги', markers: ['юрист', 'юридическ', 'адвокат', 'банкротств', 'арбитраж', 'взыскан'], audience: 'частные клиенты в сложной правовой ситуации; малый бизнес без штатного юриста; доверители по рекомендации' },
  { label: 'ателье и пошив', markers: ['ателье', 'пошив', 'швейн', 'подгонк', 'ткан'], audience: 'клиенты, которым важна посадка по фигуре: индивидуальный пошив, ремонт и подгонка; корпоративные заказы формы' },
  { label: 'кафе и еда', markers: ['кофейн', 'ресторан', 'кафе', 'доставка еды', 'пекарн', 'меню'], audience: 'гости района и офисные сотрудники рядом; заказчики доставки; постоянные гости по акциям' },
  { label: 'авто', markers: ['автосервис', 'шиномонтаж', 'автомойк', 'запчаст', 'кузовн', 'эвакуатор'], audience: 'автовладельцы района, выбирающие сервис по отзывам; корпоративные автопарки' },
  { label: 'обучение', markers: ['курс', 'обучени', 'репетитор', 'тренинг', 'вебинар', 'школа '], audience: 'специалисты, повышающие квалификацию; новички, входящие в профессию; родители, выбирающие занятия детям' },
  { label: 'недвижимость', markers: ['недвижимост', 'новостройк', 'риелтор', 'ипотек', 'аренда квартир'], audience: 'покупатели и арендаторы жилья; собственники, продающие объекты; инвесторы в недвижимость' },
  { label: 'доставка и логистика', markers: ['грузоперевозк', 'логистик', 'фулфилмент', 'доставка груз', 'перевозк'], audience: 'интернет-магазины и производители, которым нужна надёжная доставка; компании с регулярными отгрузками' },
  { label: 'ИТ и сервисы', markers: ['автоматизац', 'приложени', 'сервис', ' ии ', 'искусственн', 'crm', 'saas', 'платформ'], audience: 'руководители и специалисты, которые автоматизируют рутину; малый и средний бизнес, выбирающий сервис по демо и цене' },
  { label: 'туризм', markers: ['экскурси', 'путешеств', 'тур ', 'туры', 'отел', 'маршрут'], audience: 'путешественники, планирующие поездку заранее; компании, заказывающие корпоративные выезды' },
];
...
export function detectAudience(text        )                                             {
  ...
  for (const n of NICHES) {
    let score = 0;
    for (const m of n.markers) score += (lower.split(m).length - 1);
    if (score >= 2 && (!best || score > best.score)) best = { label: n.label, audience: n.audience, score };
  }
  return best ? { label: best.label, audience: best.audience } : null;
}
...
export async function analyzeSite(rawUrl        )                     {
  ...
  return {
    ...
    suggest: {
      ...
      niche: detectAudience(strip(body.slice(0, 400_000)) + ' ' + title + ' ' + desc)?.label ?? null,
      audience: detectAudience(strip(body.slice(0, 400_000)) + ' ' + title + ' ' + desc)?.audience ?? null,
