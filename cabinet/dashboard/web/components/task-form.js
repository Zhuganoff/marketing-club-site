...
import { api } from '../api.js?v=mtmlqkxg';
import { h, modal, KIND_LABEL, todayKey } from '../ui.js?v=mtmlqkxg';
import { IDEA_CATALOG } from '../idea-catalog.js?v=mtmlqkxg';
...
export function taskForm(app     , preset                                                                                                                      = {})              {
  ...
  const fillIdeas = () => {
    ideas.innerHTML = '';
    ideas.appendChild(h('option', { value: '' }, '— идеи из маркетинга: выберите, что попробовать —'));
    for (const g of IDEA_CATALOG[kind.value] ?? []) {
      ideas.appendChild(h('optgroup', { label: g.group }, ...g.ideas.map((i) => h('option', { value: i }, i))));
    }
    ideas.hidden = !(IDEA_CATALOG[kind.value] ?? []).length;
  };
  fillIdeas();
  kind.addEventListener('change', fillIdeas);
  ideas.addEventListener('change', () => { if (ideas.value) { title.value = ideas.value; title.focus(); } });
  ...
  const submit = async (run         ) => {
    ...
    const ok = await app.act(async () => {
      const out = await api.createTask(app.pid, { title: title.value, kind: kind.value, goal: '', asIdea: asIdea.checked, campaignId: campaign.value || null, plannedAt: planned.value || null, demo: { noSources: noSources.checked, promise: promise.checked, pii: pii.checked } });
  ...
  return h('div', { class: 'form' },
    h('label', null, 'Тип и маршрут', kind),
